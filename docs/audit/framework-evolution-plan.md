...
| ID      | Item                                                                                  | Archivos / artefactos                                                                                                | Criterio aceptacion                                                                                       | Estado                                                                                                                                                                                          |
| ------- | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FW-005  | refactor: segmentar specs por suite + dominio                                         | `tests/specs/{regression,exploratory}/<dominio>/`                                                                    | Workflows verdes; tags preservados; `release-v2.0.2/` migrado                                             | ✅ implementado (dominio `settlements`; 35 specs -> smoke/regression/exploratory por dominio; visual/ y a11y/ intactos; workflows usan --grep no paths; typecheck+lint verdes; tc-map 73 specs) |
...
| FW-007b | refactor: 4 List POMs heredan de `BaseListPage` sin redeclarar                        | `tests/pages/carrier-v2/{Client,Driver,Owner,Vehicle}ListPage.ts` + cleanup ReportsTipsPage + hardening BaseListPage | ~87 lineas duplicadas eliminadas; specs verdes; source-of-truth unico; regla virtual dispatch documentada | ✅ implementado (5 commits, branch `carrier-v2/fw-007b-list-poms-inherit`)                                                                                                                      |
| FW-007c | auditoria preventiva: 4 Settlements ListPage POMs                                     | `tests/pages/carrier-v2/Settlements{Contractor,Driver,Owner,Passenger}ListPage.ts`                                   | Verificar que heredan correctamente y no requieren refactor                                               | ⏭ NO aplica — ya estaban limpios. Ver `docs/audit/fw-007c-settlements-poms-analysis.md`                                                                                                        |
| FW-007d | auditoria preventiva: 10 POMs Detail + History                                        | `tests/pages/carrier-v2/{affiliate,settlements}/*{Detail,History}Page.ts`                                            | Verificar que heredan correctamente de BaseDetailPage                                                     | ⏭ NO aplica — ya estaban limpios. Ver `docs/audit/fw-007d-detail-poms-analysis.md`                                                                                                             |
