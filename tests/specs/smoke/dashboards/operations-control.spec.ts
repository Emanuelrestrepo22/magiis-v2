...
test.describe('@P1 @functional @migration Operations Control', () => {
  ...
  test('KPIs principales visibles (trips made, active vehicles, active drivers)', async ({
    page
  }) => {
    test.info().annotations.push({ type: 'jira', description: 'MX-5711' });
  ...
  test('Drivers Status muestra tabla + filtros (all/available/traveling/unavailable)', async ({
    page
  }) => {
    test.info().annotations.push({ type: 'jira', description: 'MX-5711' });
  ...
  test('topbar expone accion de nuevo viaje', async ({ page }) => {
    ...
    test.info().annotations.push({
      type: 'note',
      description:
        'Solo "Trip" tiene texto visible. Los iconos de historico/gestion son icon-only sin aria-label - bug de a11y a levantar con dev.'
    });
